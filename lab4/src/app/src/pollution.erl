...
add_value(CoordinatesOrName, DateTime, Type, Value, Monitor) ->
  case partition_stations(CoordinatesOrName, Monitor) of
    {[{Name, Coordinates, Values} | _], OtherStations} ->
      % Look for an existing reading with same DateTime and Type
      case lists:filter(
             fun({ReadingDateTime, ReadingType, _}) ->
               ReadingDateTime == DateTime andalso ReadingType == Type
             end,
             Values
           ) of
        [] ->
...
remove_value(CoordinateOrName, DateTime, Type, Monitor) ->
  case partition_stations(CoordinateOrName, Monitor) of
    {[{Name, Coordinates, Values} | _], OtherStations} ->
      case lists:any(
        ...
        false ->
          {error, "Reading not found for this station"};
        true ->
          Filtered = lists:filter(
                       fun({ReadingDateTime, ReadingType, _}) ->
                         not (ReadingDateTime == DateTime andalso ReadingType == Type)
                       end,
                       Values
                     ),
          UpdatedStation = {Name, Coordinates, Filtered},
          OtherStations ++ [UpdatedStation]
      end;
    _ ->
      {error, "Station not found"}
...
get_one_value(CoordinatesOrName, DateTime, Type, Monitor) ->
  case filter_stations(CoordinatesOrName, Monitor) of
    ...
    [{_, _, Values} | _] ->
      case lists:filter(
        ...
        [] ->
          {error, "No readings for this station"};
        [{_, _, Value} | _] ->
          {ok, Value}
      end
  end.
...
get_station_min(CoordinatesOrName, Type, Monitor) ->
  case filter_stations(CoordinatesOrName, Monitor) of
    [] ->
      {error, "Station not found"};
    [{_, _, Values} | _] ->
      Filtered = [Value || {_, ReadingType, Value} <- Values, ReadingType == Type],
      case Filtered of
        [] -> {error, "No readings found for this station"};
        _  -> {ok, lists:min(Filtered)}
      end
  end.
  ...
  case filter_stations(CoordinatesOrName, Monitor) of
    ...
    [{_, _, Values} | _] ->
      ...
      case Filtered of
        [] -> {error, "No readings for this station"};
        _  ->
          Mean = lists:sum(Filtered) / length(Filtered),
          {ok, Mean}
      end
  end.
...
get_daily_mean(Date, Type, Monitor) ->
  ...
  MatchedValues = [Value || {{ReadingDate, _}, ReadingType , Value} <- Values,
    ReadingType == Type,
    ReadingDate == Date],
