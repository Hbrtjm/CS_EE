...
add_value(CoordinatesOrName, Type, DateTime, Value) ->
  gen_server:call(?MODULE, {add_value, CoordinatesOrName, Type, DateTime, Value}).

remove_value(CoordinatesOrName, Type, DateTime) ->
  gen_server:call(?MODULE, {remove_value, CoordinatesOrName, Type, DateTime}).
...
handle_call({add_value, CoordinatesOrName, Type, DateTime, Value}, _From, State) ->
	case add_value(CoordinatesOrName, Type, DateTime, Value, State) of
		{error, Msg} ->
			{reply, {error, Msg}, State};
		NewState ->
			{reply, {ok, added}, NewState}
...
handle_call({remove_value, CoordinatesOrName, Type, DateTime}, _From, State) ->
	case remove_value(CoordinatesOrName, Type, DateTime, State) of
		{error, Msg} ->
			{reply, {error, Msg}, State};
		NewState ->
			{reply, {ok, removed}, NewState}
...
handle_call({get_one_value, CoordinatesOrName, DateTime, Type}, _From, State) ->
	case get_one_value(CoordinatesOrName, Type, DateTime, State) of
		{error, Msg} ->
			{reply, {error, Msg}, State};
		Value ->
...
handle_call({get_daily_mean, Type, Date}, _From, State) ->
	case get_daily_mean(Type, Date, State) of
		{error, Msg} ->
			{reply, {error, Msg}, State};
		Mean ->
			{reply, {ok, Mean}, State}
